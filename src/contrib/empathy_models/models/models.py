...
class BiEncoderAttentionWithRationaleClassification(nn.Module):
    ...
    def __init__(self, model_name, hidden_dropout_prob=0.2, rationale_num_labels=2, empathy_num_labels=3, hidden_size=768, attn_heads=1):
        super().__init__()

        self.dropout = nn.Dropout(hidden_dropout_prob)
        self.rationale_classifier = nn.Linear(
            hidden_size, rationale_num_labels)
        ...
        self.seeker_encoder = AutoModel.from_pretrained(
        ...
        self.responder_encoder = AutoModel.from_pretrained(
            # Use the 12-layer BERT model, with an uncased vocab.
            model_name,
            # Whether the model returns attentions weights.
            output_attentions=False,
    ...
    def forward(
        ...
        sequence_output_RP = sequence_output_RP + \
            self.dropout(self.attn(sequence_output_RP,
                         sequence_output_SP, sequence_output_SP))

        # (sequence_output_RP[:, 0, :]) #(torch.tanh(concat_tensor))
        ...
        if rationale_labels is not None:
            ...
            if attention_mask_RP is not None:
                ...
                active_logits = logits_rationales.view(
                    -1, self.rationale_num_labels)
                active_labels = torch.where(
                    active_loss, rationale_labels.view(-1), torch.tensor(
                        loss_fct.ignore_index).type_as(rationale_labels)
                )
                loss_rationales = loss_fct(active_logits, active_labels)
            else:
                loss_rationales = loss_fct(
                    logits_rationales.view(-1, self.rationale_num_labels), rationale_labels.view(-1))

        if empathy_labels is not None:
            loss_fct = CrossEntropyLoss()
            loss_empathy = loss_fct(
                logits_empathy.view(-1, self.empathy_num_labels), empathy_labels.view(-1))

            loss = lambda_EI * loss_empathy + lambda_RE * loss_rationales

            outputs = (loss, loss_empathy, loss_rationales) + outputs

        # (loss), (scores_empathy, scores_rationales), (hidden_states), (attentions)
        return outputs
