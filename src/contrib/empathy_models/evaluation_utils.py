...
def flat_accuracy_rationale(preds, labels, classification_labels, lens, axis_=2):
    ...
    for idx, elem in enumerate(preds_li):
        ...
        curr_acc = np.sum(np.asarray(curr_label) ==
                          np.asarray(curr_pred)) / len(curr_label)
...
def compute_f1(preds, labels, axis_=2):
    ...
    pos_f1 = f1_score(pred_flat, labels_flat,
                      average='weighted', zero_division=0)
    micro_f1 = f1_score(pred_flat, labels_flat,
                        average='micro', zero_division=0)
    macro_f1 = f1_score(pred_flat, labels_flat,
...
def compute_f1_rationale(preds, labels, classification_labels, lens, axis_=2):
    ...
    for idx, elem in enumerate(preds_li):
        ...
        macro_f1 = f1_score(curr_pred, curr_label, zero_division=0)
...
def iou_f1(preds, labels, classification_labels, lens, axis_=2, threshold=0.5):
    ...
    for idx, elem in enumerate(preds_li):
        ...
        micro_r = sum(threshold_tps.values()) / \
            len(label_spans) if len(label_spans) > 0 else 0
        micro_p = sum(threshold_tps.values()) / \
            len(pred_spans) if len(pred_spans) > 0 else 0
